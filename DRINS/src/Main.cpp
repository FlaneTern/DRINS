...
int main()
{
	...
	//std::vector<DRINS::Position> SNlocations =
	//{
	//	{15.0763, -43.2123},
	//	{12.9206, -5.03132},
	//	{24.2634, -18.2994},
				...
				//for (double multiplier = 1; multiplier <= 125001; multiplier *= 50)
				{
					...
					for (auto& mean : means)
					{
						...
						//for (double stddev = 900; stddev <= 7200; stddev += 900)
						{
							...
							std::vector<DRINS::DistributionType> failTypes =
							{
								DRINS::DistributionType::Exponential,
								//DRINS::DistributionType::Gamma,
								//DRINS::DistributionType::Lognormal,
								//DRINS::DistributionType::Weibull,
							};

							for (auto failType : failTypes)
							{
								if (failType == DRINS::DistributionType::Exponential && currentMean != currentStddev)
									continue;
									...
									for (int energyRateIterator = 0; energyRateIterator < energyRateTransfers.size(); energyRateIterator++)
									{
										for (auto& transmissionRange : transmissionRanges)
										{
											for (auto& interferenceRange : interferenceRanges)
											{
												...
												{
													DRINS::Distribution failDist(failType, currentMean, currentStddev);
													DRINS::SimulationParameters sp =
													{
													...
													//DRINS::Simulation* Si = new DRINS::Simulation(sp, SNlocations);
													DRINS::Simulation* Si = new DRINS::Simulation(sp);
