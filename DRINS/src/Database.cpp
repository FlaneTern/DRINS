...
namespace DRINS
{
    ...
    static constexpr uint32_t c_BatchRowCount = 50000 / 15;
    ...
    void Database::Insert(uint64_t simulationID, const SimulationParameters& simulationParameters, const SimulationResults& sr, const SimulationType& st)
    {
        ...
        try
        {
            static sql::PreparedStatement* statement1FTTDMA = s_Connection->prepareStatement(
                "Insert into "
                "SimulationFTTDMA(SimulationID, TotalDurationToBeTransferred, TransferTime, RecoveryTime, FailureDistributionType, FailureMean, FailureStddev, FailureParameter1, FailureParameter2, ActualTotalDuration, FinalFailureIndex, CWSNEfficiency, EnergyRateWorking, EnergyRateTransfer, MCREnabled)"
                " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

            static sql::PreparedStatement* statement1RRTDMA = s_Connection->prepareStatement(
                "Insert into "
                "SimulationRRTDMA(SimulationID, TotalDurationToBeTransferred, TransferTime, RecoveryTime, FailureDistributionType, FailureMean, FailureStddev, FailureParameter1, FailureParameter2, ActualTotalDuration, FinalFailureIndex, CWSNEfficiency, EnergyRateWorking, EnergyRateTransfer, MCREnabled)"
                " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            statement1->setBoolean(15, simulationParameters.MCREnabled);
            statement1->execute();
            s_Connection->commit();
